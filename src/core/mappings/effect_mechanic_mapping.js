...
EFFECT_MECHANIC_MAPPING['ADD_MODIFIER_TO_SELF'] = async function (duel, card, effect, targetCard, opts = { modifierData }) {
  let modifier = new Modifier(opts.modifierData);
  modifier.setLinkedEffect(effect);
  await duel.operationAddDuelistModifier(card.getControler(), modifier);
}

EFFECT_MECHANIC_MAPPING['REMOVE_MODIFIER_TO_SELF'] = async function (duel, card, effect, targetCard, opts = { modifierId }) {
  await duel.operationRemoveDuelistModifier(card.getControler(), opts.modifierId);
}
...
EFFECT_MECHANIC_MAPPING['REMOVE_CARD_MODIFIER'] = async function (duel, card, effect, targetCard, opts = { modifierId }) {
  await duel.operationRemoveCardModifier(targetCard, opts.modifierId);
}
