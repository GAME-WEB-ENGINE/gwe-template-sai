...
class UIBoard extends GWE.UIWidget {
  ...
  update() {
    if (this.duel) {
      let duelists = this.duel.getDuelists();

      for (let slot of this.slots) {
        if (slot instanceof UICardSlot) {
          slot.setCard(duelists[slot.getDuelistIndex()].getCard(slot.getLocation(), slot.getIndex()));
        }
        else {
          slot.setCards(duelists[slot.getDuelistIndex()].getZone(slot.getLocation()));
        }
      }
  ...
  onKeyDownOnce(data) {
    ...
    else if (data.key == GWE.InputKeyEnum.UP) {
      this.nextFocus('UP', true);
    }
    else if (data.key == GWE.InputKeyEnum.DOWN) {
      this.nextFocus('DOWN', true);
    }
    else if (data.key == GWE.InputKeyEnum.LEFT) {
      this.nextFocus('LEFT', true);
    }
    else if (data.key == GWE.InputKeyEnum.RIGHT) {
